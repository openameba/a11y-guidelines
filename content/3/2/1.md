...
視覚障害や運動障害のあるユーザーは、キーボードなどで操作を行うことがある。フォーカスで次の「[悪い例](/a11y-guidelines/3/2/1#悪い例)」のような変更が起きると、操作を完了できない恐れがある。ユーザー自身が、クリックやエンターキーでの操作、タブキーや十時キーでの移動を行うのが望ましい。
...
- 分割されたカード番号の入力欄で、4桁入力した瞬間に自動で次の欄にフォーカスが移る（American Express Cardなどは区切りが4桁ではなく5桁、6桁の箇所もあるので自動的に移動させると入力できなくなる場合がある）
