...
```
![pattern属性を用いた入力フォームの例。「ameba blog」と「ameblo」のみ送信が可能になり、それ以外の文字列を入力して送信すると「指定されている形式で入力してください。」などのvalidationエラーの文言が入力欄付近に表示される。](/a11y-guidelines/img/3/3/3/3.3.3_1.png)

`pattern`属性に含めた文字列（正規表現可）のみ送信可能になる。
...
![required属性を用いた入力フォームの例。入力が必須となっており、未入力の状態で送信すると「このフィールドを入力してください。」などのvalidationエラーの文言が入力欄付近に表示される。](/a11y-guidelines/img/3/3/3/3.3.3_2.png)
